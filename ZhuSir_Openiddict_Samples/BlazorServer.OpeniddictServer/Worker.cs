using BlazorServer.OpeniddictServer.Data;
using Microsoft.AspNetCore.Identity;
using OpenIddict.Abstractions;
using Quartz;
using static OpenIddict.Abstractions.OpenIddictConstants;

namespace BlazorServer.OpeniddictServer
{
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            if (await manager.FindByClientIdAsync("blazor-Server-client") is null)
            {
                ...
                await manager.CreateAsync(new OpenIddictApplicationDescriptor
                {
                    ...
                    {
                        new Uri("http://localhost:8000/signout-callback-oidc"),
                        new Uri("https://localhost:8001/signout-callback-oidc"),
                    },
                    RedirectUris =
                    {
                        new Uri("http://localhost:8000/signin-oidc"),
                        new Uri("https://localhost:8001/signin-oidc")
                    },
                    Permissions =
                    {
